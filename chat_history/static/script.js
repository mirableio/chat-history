    ...
    if (isResolved) {
        ...
        return `
            <div class="msg-block msg-block-asset msg-block-asset-image">
                ...
                <a class="msg-asset-image-link" href="${assetUrl}" target="_blank" rel="noopener noreferrer">
                    <img class="msg-asset-image" src="${assetUrl}" alt="${escapeHtml(altText)}" loading="lazy" />
                </a>
                ${metaHtml}
            </div>
        `;
    ...
    if (isResolved) {
        return `
            <div class="msg-block msg-block-asset msg-block-asset-audio">
                <div class="msg-block-label">${label}</div>
                <audio class="msg-asset-audio-player" controls preload="none" src="${assetUrl}"></audio>
                ${metaHtml}
            </div>
        `;
    }
...
function renderClaudeAttachmentBlock(block) {
    const data = block?.data && typeof block.data === "object" ? block.data : {};
    const text = String(block?.text || "");
...
function renderMessageBlock(block) {
    ...
    if (type === "attachment") {
        return renderClaudeAttachmentBlock(block);
    }
...
function shouldHideFileBlockAsDuplicate(block, attachmentByIndex) {
    ...
    // Hide duplicate if names match or one side has no useful name (the common Claude case).
...
function renderMessageBlocks(blocks) {
    ...
    return filteredBlocks.map(block => renderMessageBlock(block)).join("");
}
